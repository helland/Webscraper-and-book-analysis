...
import webscraper.scraper_book as scraper
import webscraper.scraper_toc as setup
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.all_books = [] # list of all available books from the database
        self.populate_book_list() # add all books in the database to the "Books" listwidget items as well as the "all_books" list
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.comboBox_analysis.setItemText(11, _translate("MainWindow", "Find the book that is most different from the others"))
    ...
    # Add input book to the "Books" list widget
    ...
    # get all stored books into the all_books list and populate the "Books" listwidget with them
    def populate_book_list(self):
        self.listWidget_books.clear()                                                       # remove all items in order to repopulate the Books listwidget
        self.all_books = []                                                                 # remove all items in order to repopulate the all_books object list
        books_in_database = utils.get_stored_and_processed_books(self.db_config)            # get all basic information on all books stored and encoded in the database
        ...
        for a_book in sorted_books:
            self.all_books.append(book.Book(int(a_book['Id']), self.db_config))     
            self.add_book_to_list(self.all_books[-1])  # add the books taken from the database to the list       
    ...
    def add_selected_books(self):
        ...
        # Close search and reassure user that something is happening after they clicked "ok"
        display_text = "<br>Scraping books from Gutenberg...<br>"
        self.textEdit_mainField.setText(display_text) 
        self.hide_search_widgets()
        
        selected_books = []
        for item in selected_items:
            for a_book in self.search_results:
                if a_book['Title'] == item.text():
                    selected_books.append(a_book)
                    break
        ...
        for book_data in selected_books:
            try:
                ...
                book_data['Title'], book_data['Author'], book_data['ReleaseDate'], book_data['Language'], book_data['text'], book_data['WordCount'], book_data['CharacterCount'] = scraper.gutenberg_book_scraper(book_data['SourceWebsite'])
        ...
        display_text += "<br>Storing books in database. Please wait...<br>"
        self.textEdit_mainField.setText(display_text) 
        scraper.store_books_in_database(scraped_books, params.db_config)

        # Add the newly stored book(s) to the Books listWidget
        for a_book in scraped_books:
            new_book_id = utils.get_book_ids_with_title(a_book['Title'], False, self.db_config) # get the new book's id from the database
            new_book = book.Book(new_book_id, self.db_config)                                   # create book object.  
            self.all_books.append(new_book)                                                     # add the book object to the local list of loaded books
            self.populate_book_list() #self.add_book_to_list(self.all_books[-1])                # add book to listwidget 
        
        display_text += "<br>The following books were added to the database:\n" + "\n".join(added_books_titles)     
        self.textEdit_mainField.setText(display_text)    
        # Show popup message
        QtWidgets.QMessageBox.information(self.centralwidget, "Books Added", "The following books were added to the database:<br> " + "<br>".join(added_books_titles))
    ...
    def get_selected_books(self):    
        selected_items = self.listWidget_books.selectedItems()                                      # selected books
        selected_titles = {item.text() for item in selected_items}                                  # titles of the selected books
        selected_books = [book for book in self.all_books if book.title in sorted(selected_titles)] # book objects matching the selected books in title
        return selected_books   # TODO: add a way to also take into account the selected book's position in the list so that two books with the same title isn't confused for one another
    ...
    def enter_button(self):
        ...
        if not books: 
            self.textEdit_mainField.setText("<br>No books selected.")  
        ...
        match book_analysis:
            ...
            case 1:                        
                ...
                text_words = analysis.cipher_decoder(most_frequent_word, book.language, self.db_config) # decode cipher back to text
            ...
            case 2:                         
                ...
                text_words = analysis.cipher_decoder(most_frequent_words, book.language, self.db_config) # decode cipher back to text
            ...
            case 3:
                ...
                for book in books:
                    ...
                    count.append(len(words))    # count how many unique words were found
                    
                text_words = analysis.cipher_decoder(unique_words, book.language, self.db_config) # decode cipher back to text
                displayed_text = mField.books_words(books, text_words) + f"Total: {count} unique words!<br>"
                self.textEdit_mainField.setText(displayed_text)      
                 
            # generate a wordcloud for selected books
            case 4:
                for book in books:
                    book_text = analysis.cipher_decoder(self.get_encoded_text(book), book.language, params.db_config)
                    delimiter = " "
            ...
            case 5:
                ...
                text_sentences = analysis.cipher_decoder(longest_sentences, book.language, self.db_config) # decode cipher back to text
            ...
            case 6:
                ...
                text_sentences = analysis.cipher_decoder(shortest_sentences, book.language, self.db_config) # decode cipher back to text
                displayed_text = mField.books_sentences(books, text_sentences, sentences_per_book)
                self.textEdit_mainField.setText(displayed_text)                   
            ...
            case 8: 
                try:
                    cnx = mysql.connector.connect(**self.db_config) # connect to db only once before the loop, to find word ids without making one connection per book
                    cursor = cnx.cursor()
                    sentences_starting_with_N, sentences_per_book = [], [] 
                         
                    for book in books:
                        word = self.get_id_from_word(self.get_N_input(), book, cursor)  # unfortunately, this needs to be run once for every book, because the books might be in differnet languages, and thus, the id of the word might differ
                    ...
                    text_sentences = analysis.cipher_decoder(sentences_starting_with_N, book.language, self.db_config) # decode cipher back to text
                    displayed_text = mField.books_sentences(books, text_sentences, sentences_per_book)
                    self.textEdit_mainField.setText(displayed_text) 
                except mysql.connector.Error as err:
                    print(f"Error: {err}") #Error finding the id of the word '{word}' from the table {book.language_name.lower()}dictionary: 
                finally:
                    if cnx:
                        cnx.close()                 
            ...
            case 9: 
                longest_words = []  
                for book in books:
                    longest_word = analysis.find_longest_words(self.get_encoded_text(book), book.language, self.db_config)
                    longest_words.append(longest_word)
                print(longest_words)
                text_longest_words = analysis.cipher_decoder(longest_words, book.language, self.db_config) # decode cipher back to text
                print(text_longest_words)
            ...
            case 10:
                ...
                full_book_text = analysis.cipher_decoder(books[0].text, books[0].language, self.db_config) # decode a full book back to text      
                displayed_text = mField.indices_words(indices_with_same_words, full_book_text) # get only words at indices and format them
                self.textEdit_mainField.setText(displayed_text)  
            
            # Find the book that is most different in its word use from the other selected books
            case 11:
                most_deviant_book, highest_deviation = analysis.find_book_with_highest_deviation(books)
                self.textEdit_mainField.setText(f"<br><h2>{most_deviant_book.title}</h2><br>Is the book with the highest deviation in its use of words among the selected books.<br><br>Deviation: {round(highest_deviation,3)}")  
    ...
    # Take the input database details and add them to local variables, as well as initialize the database & tables if it hasn't already been done
    def get_db_config(self, dialog, host_input, user_input, password_input, database_input, use_pure_checkbox):
        ...
        self.textEdit_mainField.setText(f"<h1>Database configuration implemented.</h1><br>{self.db_config}<br><h3> book_project database and relevant tables were added if they did not already exist.</h3>")

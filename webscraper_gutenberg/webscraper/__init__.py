...
if __name__ == "__main__":    
    info = {
        "Any text that does not match an instruction": "This will attempt to encode and upload a book by this title to the database. It will only work if the book can be found in the Gutenberg table of content generated by the setup.",
        "init123": "Create the basic tables in the database you want your books to be stored.",
        "setup123": "set up the table of content to show which books are possible to add and get the relevant links needed to add them.",
        "exit123": "End program",
        "Leave the command line empty": "Enter book analysis mode."
        }
    input_data = ' '
    ...
    while input_data != 'exit123':
        ...
        # Initialize database
            ...
            init.create_database(params.db_config)
            init.create_tables(params.db_config)
        ...
        # Run data analysis if the command is left blank
            ...
            if does_book_exist[0]['StoredAndProcessed'] == True:                       #for simplicity's sake, only look at the first book found.
                ...
                sentence_length = analysis.calculate_average_sentence_length(book.text, params.punctuations)
                sentences_starting_with = analysis.find_sentences_starting_with(book.text, params.punctuations, 448) # the word "Be"
                longest_word = analysis.find_longest_words(book.text, book.language, params.db_config)
               
                # convert encoded text back to string values.
                most_frequent_word = analysis.cipher_decoder([most_frequent_word], book.language, params.db_config)
                most_frequent_words = analysis.cipher_decoder(most_frequent_words, book.language, params.db_config)
                words_used_only_once = analysis.cipher_decoder(words_used_only_once, book.language, params.db_config)
                sentences = analysis.cipher_decoder(sentences, book.language, params.db_config)
                longest_sentence = analysis.cipher_decoder(longest_sentence, book.language, params.db_config)
                shortest_sentence = analysis.cipher_decoder(shortest_sentence, book.language, params.db_config)
                sentences_starting_with = analysis.cipher_decoder(sentences_starting_with, book.language, params.db_config)
                longest_word = analysis.cipher_decoder(longest_word, book.language, params.db_config)
                ...
                print(f"sentences starting with the word 'Be':  {len(sentences_starting_with)}")
            ...
            if len(input_books) == 1:
                ...
                if input_books[0]['StoredAndProcessed'] == True:
                    ...
                    scraper.store_books_in_database((book,), params.db_config)

...
def update_word_entry_counts(cursor):  
    #try:
...
def get_books_with_title(title, get_all, cursor):
    #try:
    #    cnx = mysql.connector.connect(**db_config)
...
# Returns a list of books with input title as dictionaries.
def get_book_ids_with_title(title, get_all, cursor):
    #try:
    #    cnx = mysql.connector.connect(**db_config)
    #    cursor = cnx.cursor(dictionary=True)  
...
# Returns a list of books with input title as dictionaries.
def get_stored_and_processed_books(cursor):
    #try:
    #    cnx = mysql.connector.connect(**db_config)
    #    cursor = cnx.cursor(dictionary=True)  
...
def get_non_alphanumerics(dictionary_name, cursor): # NOTE: this function was added to Book object init. It might not be useful anymore, but I'm keeping it in case it might be needed outside a book object
    #try:
    #    cnx = mysql.connector.connect(**db_config)
    #    cursor = cnx.cursor()
